...
export async function POST(req: NextRequest) {
  try {
    ...
    if (messagesError) {
      return NextResponse.json(
        {
          error: `Could not load chat messages: ${messagesError.message}`,
        },
        { status: 500 }
      );
    }
    ...
    const { data: meetingSummaries, error: meetingsError } =
      await supabase
        .from("call_summaries")
        .select(
    ...
    if (meetingsError) {
      return NextResponse.json(
        {
          error: `Could not load meeting summaries: ${meetingsError.message}`,
        },
        { status: 500 }
      );
    }
    ...
    const chatConversation = (messages ?? [])
      ...
      .map((message) => {
        const sender =
          profileMap[message.sender_id]?.name ??
          "Employee";
...
${chatConversation || "No chat messages found."}
...
${meetingConversation || "No meeting summaries found."}
`;
    ...
    const { data: savedMom, error: saveError } =
      await supabase
        .from("daily_moms")
        .upsert(
          {
            ...
            mom_date: momDate,
            created_by: user.id,
            title: `Daily MOM — ${momDate}`,
            content: momContent,
            source_message_count:
              chatCount + meetingCount,
            updated_at:
    ...
    if (saveError) {
      return NextResponse.json(
        {
          error:
            `Could not save Daily MOM: ${saveError.message}`,
        },
        { status: 500 }
      );
    }
